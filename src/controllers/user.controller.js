...
const loginUser = asyncHandler(async(req,res)=>{
    ...
    if(!isPasswordValid){
        throw new ApiError(401,"Invalid user credentiasl")
    }
...
const refreshAccessToken = asyncHandler(async(req,res) => {
    try {
        ...
        if(incomingRefreshToken !== user?.refreshToken){
            throw new ApiError(401,"Refresh token in expires or used")
        }
        ...
        .cookie("refreshToke",newRefreshToken)
        .json(
            new ApiResponse(
                200,
...
const updateUserCoverImage = asyncHandler(async(req,res) =>{
    const coverImageLocalPathh=req.file?.path
